...
static void
fm_directory_view_chose_application_callback (GAppInfo *application, 
					      gpointer callback_data)
{
			 ...
			 gpointer user_data)
{
	...
	files.prev = NULL;
	files.data = file;
	fm_directory_view_launch_application (app, &files, view);
}
...
fm_directory_view_display_selection_info (FMDirectoryView *view)
{
	...
	if (non_folder_count != 0) {
		...
		if (non_folder_size_known) {
			...
			size_string = g_file_size_format_for_display (non_folder_size);
